...
shuffle(emojiList);
    ...
    // Map to grid cell
    const col = Math.floor(x / (memCanvas.width / canvasCol));
    const row = Math.floor(y / (memCanvas.height / canvasRow));

    console.log(`Click at (${x}, ${y}) → Cell [${row}, ${col}]`);

    // … then continue with your reveal/match logic
});
});
        const emojiIndex = row * cols + col;
