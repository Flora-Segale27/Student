...
<script>
    ...
    function drawGrid(GRID_COLS, GRID_ROWS) {
    ...
    function drawEmojis(GRID_COLS, GRID_ROWS, emojis) {
...
memCanvas.addEventListener('click', (event) => {
    // Don’t allow more than 2 revealed at once
    ...
    const cols = 6;
    const rows = 6;
    const col = Math.floor(x / (memCanvas.width / cols));
    const row = Math.floor(y / (memCanvas.height / rows));
    const emojiIndex = row * cols + col;
   ...
   const emoji = revealEmojiAt(col, row, emojiList, 6, 6);
